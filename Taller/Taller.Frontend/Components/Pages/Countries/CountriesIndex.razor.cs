...
public partial class CountriesIndex
{
    ...
    private async Task ShowModalAsync(int id = 0, bool isEdit = false)
    {
        ...
        if (isEdit)
        {
            ...
            {
                { "Id", id }
            }; dialog = await DialogService.ShowAsync<CountriesEdit>("Editar país", parameters, options);
        }
        else
        {
            dialog = await DialogService.ShowAsync<CountriesCreate>("Nuevo país", options);
        }
