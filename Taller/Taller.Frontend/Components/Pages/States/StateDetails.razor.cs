...
public partial class StateDetails
{
    ...
    private async Task ShowModalAsync(int id = 0, bool isEdit = false)
    {
        ...
        if (isEdit)
        {
            ...
            {
                { "Id", id }
            }; dialog = await DialogService.ShowAsync<CitiesEdit>("Editar Ciudad", parameters, options);
        }
        else
        {
            var parameters = new DialogParameters
            ...
            dialog = await DialogService.ShowAsync<CitiesCreate>("Nuevo Ciudad", parameters, options);
        }
